...
extension Shared where Self: UIViewController {
    ...
    static func shared() -> Self {
        guard let path = Bundle.main.resourcePath else {return self.init()}
        guard let contents = try? FileManager.default.contentsOfDirectory(atPath: path) else {return self.init()}
        
        var SBLists = contents.filter {$0.hasSuffix(".storyboardc")}
        SBLists = SBLists.map {($0 as NSString).deletingPathExtension as String}
        if let mainSB = Bundle.main.object(forInfoDictionaryKey: "UIMainStoryboardFile") as? String {
            SBLists.append(mainSB)
        }
        
        for sb in SBLists {
            let storyboard = UIStoryboard(name: sb, bundle: nil)
            if let VCLists = storyboard.value(forKey: "identifierToNibNameMap") as? [String : String] {
                if let matchedName = VCLists[String(describing: self)] {
                    return storyboard.instantiateViewController(withIdentifier: matchedName) as! Self
                }
            }
        }
        return self.init()
